...
MAX_ATTEMPTS=60
ATTEMPT=1
until mysql -u"$DRUPAL_DB_USER" -p"$DRUPAL_DB_PASSWORD" "$DRUPAL_DB_NAME" -e "SELECT 1" 2>/tmp/mysql_error.log; do
...
# Vérifier le montage du volume (avant la connexion DB)
